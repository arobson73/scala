//Note i built this with sbt, by having State.scala, Par.scala,
//Stream.scala in same folder. then i imported each of them
//into the REPL. 
//import Stream._
import State._
import Par._ 
...
//:paste
//noddy way to create a prop. first create function for run
//val f = (x:MaxSize,y:TestCases,s:RNG) => {if (y > 2) Passed else Proved}
//val o1 = Prop(f)
//o1.run(100,1,Simple(3))   gives Proved
//o1.run(100,6,Simple(4))   gives Passed
case class Prop(run: (MaxSize,TestCases,RNG) => Result) {

 //val f1 = (x:MaxSize,y:TestCases,s:RNG) => {if (y > 10) Passed else Proved}
 //val o1 = Prop(f)
 //val o2 = Prop(f1)
 //(o1 && o2).run(100,20,Simple(3))
 //one way of looking at this is its just one big pattern match
 //Prop {(mx,n,rn) => ..
 //o1 is an instance. we call its run method with actual values to run
 // it. the && method "joins" 2 Props together. those values passed in the run
 // are then actual run by the first run for the lhs prop, to get the first result, which is then
 // matched. if the match is passed or proved then we invokde the next prop with those values 
  def &&(p: Prop) = Prop {
...
///NOTE in general the methods inside of a case class are 
//internal methods, which you don't explicity use. There
//are use instead by the methods you define in the object
//Gen
//def f(rng:RNG):(Int,RNG) = rng.nextInt
//val s = State(f)
//val g = Gen(s)
//g.map(x => x*2).sample.run(Simple(3))
...
//Gen.choose(3,10).sample.run(Simple(100))
 ...
 //how is this a union
   ...
   */
  //the first map in here get the values from the gen list. these values
  //are then fed to another map to conver them to chars.
